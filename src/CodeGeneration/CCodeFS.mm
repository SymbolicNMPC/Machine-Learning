...
CCodeFS := proc(States, Costates, Fxuw, dFxuw, CostFunction, JCF, dimx, dimu, dimw, N, Nc)
	local Fxu, dFxu, Fxuc, dFxuc, CF, JacobianCF, CFc, JCFc, Code_str, Include, IncludeLines, i, j, defs, WL, optionsC,
    ...
    # Vector field
    fir := ToInert(eval(Fxuw));
    ReturnStatement := indets(fir,specfunc(':-_Inert_LIST'))[1];
    Fxu:=FromInert(subs(ReturnStatement=':-_Inert_RETURN'(ReturnStatement),fir)):

    # Jacobian of the vector field
    dFxu:=codegen:-makeparam('grd',dFxuw):

    # Cost function
    fir := ToInert(eval(CostFunction));
    ReturnStatement := indets(fir,specfunc(':-_Inert_STATSEQ'))[1];
    CF := FromInert(subs(ReturnStatement=':-_Inert_STATSEQ'(op(ReturnStatement)[1..-2],':-_Inert_RETURN'(':-_Inert_INTPOS'(0))), fir));

    # Jacobian of the cost function (optimality conditions)
    JacobianCF := JCF:
         
    # Gradient descent
    GD := parse(StringTools:-SubstituteAll(convert(eval(':-OptimCode:-GradientDescentFS'),string),"OptimCode:-","C")):
    ...
    # Main MPC
    mainMPC := parse(StringTools:-SubstituteAll(convert(eval(':-OptimCode:-MPCtrlFS'),string),"OptimCode:-","C"));

    WL := interface(warnlevel);
    interface(warnlevel=0);

    Fxuc := CodeGeneration:-C(Fxu,optionsC);
    dFxuc := CodeGeneration:-C(dFxu,optionsC);
    CFc := CodeGeneration:-C(CF, ':-declare'=[':-U'::'Vector'(n), ':-Xn'::'Vector'(dimx), ':-f_X'::'vector'(n), ':-flag'::'integer', 
    ...
    Code_str := StringTools:-SubstituteAll(Code_str,"GD","GradientDescentFS"):
    Code_str := StringTools:-SubstituteAll(Code_str,"CGradientDescentFS","GradientDescentFS"):
